...
| Point | 회원 보유 포인트 규칙 | `member.point` DB 컬럼은 유지하되, 충전/차감 규칙은 `Point` 값 객체로 분리 (향후 포인트 충전/사용 이력 및 만료 정책 등 독립적 확장을 고려하여 최상위 도메인 패키지로 설계) | `PointTest` |
...
#### 1. Wish -> Catalog 의존성 격리 (Port/Adapter 패턴 적용)
- 핵심 도메인(Core Subdomain)인 `catalog`는 서브 도메인(Supporting Subdomain)인 `wish`를 인지해서는 안 됩니다.
- 이를 위해 `wish.application` 내부에 `WishProductPort` 인터페이스를 정의하고, `catalog` 도메인과 통신하여 정보를 제공하는 `CatalogWishProductAdapter` 구현체는 `wish.infrastructure`에 배치하였습니다.
- 이로 인해 의존성 방향은 항상 단방향(`wish` -> `catalog`)으로 흐르며, `catalog`는 `wish` 도메인의 요구사항(예: 위시리스트 조회 성능 개선)으로부터 완벽하게 격리됩니다.

#### 2. Point 도메인 최상위 패키지 분리
- 현재 포인트는 '회원의 포인트 차감' 등의 종속적인 속성처럼 취급되지만, 비즈니스 관점에서 독립적인 생명주기와 규칙을 가지는 서브 도메인으로 정의됩니다.
- 향후 충전/사용 거래 이력 추적, 만료일자 관리, 포인트 소급 등 독자적인 비즈니스 요건 확장이 예정되어 있으므로, `member` 패키지 하위로 결합도를 높이기보다 최상위 패키지(`gift.point`)로 분리해 유지보수성과 확장성을 높였습니다.
...
- `docs/adr/0007-pr-review-refactorings-and-concurrency.md`: PR 리뷰 피드백에 따른 도메인 격리 강화 및 동시성 제어 결정
- `docs/adr/0008-entity-internal-validation.md`: 값 객체(Value Object)를 제거하고 도메인 엔티티 내부에서 직접 검증을 처리한다
